...
def encrypt_aes_128_cbc(plaintext: str, key: bytes) -> str:
    # 1. Generar Vector de Inicialización (IV) dinámico de 16 bytes
    iv = os.urandom(16)
    
    # 2. Configurar el cifrador
    cipher = Cipher(algorithms.AES(key), modes.CBC(iv), backend=default_backend())
    encryptor = cipher.encryptor()
    
    # 3. Añadir relleno PKCS7 para alinear el texto plano a bloques de 16 bytes
    ...
    # 5. Prefijar el IV al criptograma y luego codificar en Base64
...
def main():
    ...
    while True:
        # 1. Selecciona un auto al azar y genera valores
